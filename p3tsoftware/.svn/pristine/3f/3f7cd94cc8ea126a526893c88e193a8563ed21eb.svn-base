...
public class GroupEval {
	...
	while ((c = g.getopt()) != -1) map.get(c).execute();
	...
	public static void printHTMLMethodA(int nexpected, int expected, int[] foundVect, int[] correctVect, long[] timeVect, int k, String format,
	...
	for ( Vector test : result ) {
	    ...
	    f.nextElement();
	    for( int k = 0 ; f.hasMoreElements() ; k++) {
		printHTMLMethodA(nexpected, expected, foundVect, correctVect, timeVect, k, format, writer, fsize, formatter, f);
	    }
	    writer.println("</tr>");
	}
