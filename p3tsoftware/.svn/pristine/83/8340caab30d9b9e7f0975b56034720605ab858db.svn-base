...
public class CacheImpl {
	...
	if ( u1 != null ) {
	    for ( Alignment al : ontologyTable.get( u1 ) ) {
			try {
				...
				else if ( checkOntologyURI(al, u2)) results.add( al );
				//    }
	...
	private boolean checkOntologyURI(Alignment al, URI u){
		boolean flag = false;
		if(al.getOntology2URI().equals( u ) || al.getOntology1URI().equals( u )){
				flag = true;
		}
		return flag;
	}
    ...
    protected Alignment retrieveDescription( String id ){
		...
		try {
			...
			getBasicOntologyMetadata(st, result);

			// Get ontologies
			getOntologies(st, result);
	...
	private void getBasicOntologyMetadata(Statement st, URIAlignment result) throws Exception{
		ResultSet rs = st.executeQuery( "SELECT * FROM alignment WHERE id = '" + id  +"'" );
		while( rs.next() ) {
	...
	private void getOntologies(Statement st, URIAlignment result) throws SQLException, URISyntaxException {
		ResultSet rs = st.executeQuery( "SELECT * FROM ontology WHERE id = '" + id  +"'" );
		while(rs.next()) {
			if ( rs.getBoolean("source") ) {
				getOntologiesFromSource1(rs, result);
			} else {
				getOntologiesFromSource2(rs, result);
			}
		}
	}
	
	private void getOntologiesFromSource1(ResultSet rs, URIAlignment result) throws SQLException, URISyntaxException {
	...
	private void getOntologiesFromSource2(ResultSet rs, URIAlignment result) throws SQLException, URISyntaxException {
    ...
    public Set<Alignment> getAlignments( URI uri1, URI uri2 ) {
		...
		if ( uri2 != null ){
			getAlignmentsFromURI2(uri2, potential);
		} 
		if ( uri1 != null ) {
			if(!getAlignmentsFromURI1(uri1, potential)) return potential;
		} else { result = potential; }
		return result;
    }
	
	private void getAlignmentsFromURI2(URI uri2, Set<Alignment> potential){
	...
	private boolean getAlignmentsFromURI1(URI uri1,  Set<Alignment> potential){
		boolean flag=true;
    ...
    public void storeAlignment( String uri ) throws AlignmentException, SQLException {
		...
		for( int i=0; i < 3 ; i++ ) {
			...
			try {
				...
				// store dependencies
				storeDependencies(st, alignment, id);
			
				for( Cell c : alignment ) {
					String cellid = storeCell(st, alignment, c, id);
					if ( cellid != null && !cellid.equals("") && c.getExtensions() != null ) {
						// Store extensions
						storeExtensions(st, c, cellid);
					}	
				}
			} catch ( SQLException sex ) {
				logger.warn( "SQLError", sex );
	...
	private void storeDependencies(Statement st, BasicAlignment alignment, String id){
		String query = null;
	...
	private void storeExtensions(Statement st, Cell c, String cellid){
		String query = null;
	...
	private String storeCell(Statement st, BasicAlignment alignment, Cell c, String id){
		String query = null;
		String cellid = null;
    ...
    */
	...
	public void updateDatabase() throws SQLException, AlignmentException {
		...
		if (version < VERSION) {
			if (version >= 302) {
				if (version < 310) {
					updateDatabaseVersion310(st);
				}
				// Nothing to do with 340: subsumed by 400
				if (version < 400) {
					updateDatabaseVersion400(st);
				}
				if (version < 450) {
					updateDatabaseVersion450(st);
				}
				// ALTER version
	...
	private void updateDatabaseVersion310(Statement st) throws SQLException {
		logger.info("Upgrading to version 3.1");
		// ALTER database
		renameColumn(st, "extension", "method", "val", "VARCHAR(500)");
		// case mysql
		// st.executeUpdate("ALTER TABLE extension CHANGE method val
		// VARCHAR(500)");
		...
		while (rse.next()) {
			...
			// logger.trace(" Treating tag {} of {}", tag,
			// rse.getString("id"));
			if (!tag.equals("")) {
				int pos;
				String ns;
				...
				st2.executeUpdate("UPDATE extension SET tag='" + name + "', uri='" + ns + "' WHERE id='"
						+ rse.getString("id") + "' AND tag='" + tag + "'");
			}
		}
	}

	private void updateDatabaseVersion400(Statement st) throws SQLException {
	...
	private void updateDatabaseVersion450(Statement st) throws SQLException {
		...
		while (rse.next()) {
			...
			st2.executeUpdate("INSERT INTO ontology (id, uri, source, file) VALUES ('" + rse.getString("id") + "','"
					+ rse.getString("uri1") + "','1','" + rse.getString("file1") + "')");
			st2.executeUpdate("INSERT INTO ontology (id, uri, source, file) VALUES ('" + rse.getString("id") + "','"
					+ rse.getString("uri2") + "','0','" + rse.getString("file2") + "')");
		}
		...
		st.executeUpdate(
				"UPDATE extension SET val=( SELECT e2.val FROM extension e2 WHERE e2.tag='cached' AND e2.id=extension.id ) WHERE tag='stored' AND val=''");
		// We should also implement a clean up (suppress all
		// starting with http://)
	}
