...
public class MaxCoupling extends AbstractExtractor {
	...
	boolean transpose=transpose(s1, s2, src, trg);
	...
	boolean allEquals=areAllEquals(values);
    ...
    private boolean areAllEquals(double[][] values ){
    ...
    private <O> boolean transpose(O[] s1, O[] s2, Set<? extends O> src, Set<? extends O> trg){
