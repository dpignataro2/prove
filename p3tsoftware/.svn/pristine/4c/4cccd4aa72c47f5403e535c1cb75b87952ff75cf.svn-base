...
public class MeasureFactory {
	...
	public Object getInstance(String name, Node current, String basePackage) {
		int idx = calculateIdx(name);

		Node n = fillNode(current, basePackage, idx, name);
	...
	private int calculateIdx(String name){
		int idx = 0;
		while (idx < name.length() && name.charAt(idx) != '(' && name.charAt(idx) != ')' && name.charAt(idx) != ',')
			idx++;
		return idx;
	}
	
	private Node fillNode(Node current, String basePackage, int idx, String name){
		Node n = null;
		if (idx > 0) {
			n = new Node();
	...
	private Object newInstance(Node current) {
		...
		int nbParam = calculateNbParam(Node current);
		...
		fillConstructor(current, nbParam, cs, params);
		nullInstance(current, cs);
		if (mCache != null)
	...
	private int calculateNbParam(Node current){
		return current.children == null ? 0 : current.children.size();
	}
	
	private void nullInstance(Node current, Class<?>[] cs) throws OntoSimException{
		if (current.instance == null) {
			...
			sb.append(')');
			throw new OntoSimException("No Constructor " + current.classe.getSimpleName() + sb.toString()
					+ " for class " + current.classe.getCanonicalName());
		}		
	}
	
	private void fillConstructor(Node current, int nbParam, Class<?>[] cs, Object[] params){
